...
public class MusicOrganizer {
    ...
    public void loadSongs(String path, String extension, String[] nombresTemas) {
        //camino, extension y lista de nombres que se tienen que recorrer.(for each).
    	for (String tema:nombresTemas) {
    		fileNames.add(path+tema+extension);//hay que concatenar la dir de la mp3.
    		titles.add(tema);
    ...
    private boolean validIndex(int index) {
    	...
    	if (index<=0) {
    	 ...
    	 else if (index>fileNames.size()) {// tmb se podria usar titles.size
    ...
    public void printFileName(int index) {
    	//PENDIENTE DE IMPLEMENTAR
    	 ...
    	 if (validIndex(index)){
    		 System.out.println(fileNames.get(index));//No se puede usar colchete con arrayList
    ...
    public void addSong(String path, String extension, String title) {
    	//PENDIENTE DE IMPLEMENTAR
    		fileNames.add(path+title+extension);//hay que concatenar la dir de la mp3.
    		titles.add(title);
    }
